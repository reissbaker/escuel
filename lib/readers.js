...
function findSQLFiles(directory, whitelist) {
  ...
  return _.filter(files, (filename) => {
    ...
    if(filtering) {
      const hasNoExtension = _.has(whiteset, path.basename(filename, '.sql'));
      const hasExtension = _.has(whiteset, path.basename(filename, '.sql') + '.sql');
      return hasNoExtension || hasExtension;
    }
